...
lf <- list.files("PontOut", full.names = TRUE)

coo <- import_jpg(lf, auto.notcentered = TRUE, 
                  fun.notcentered = NULL, 
                  threshold = 0.5)

database <- read.csv("Pontnewydd.csv")
...
stack(shapenorm, title = "Stack: Normalised Pontnewydd Outlines")
...
stack(shapenorm2, title = "Stack: Normalised Pontnewydd Outlines")
...
tree <- hclust(dist(database.expanded[,12:54]))
...
ggtree(tree, branch.length = "none") %<+% reference +
  geom_tippoint(size = 3) +
  geom_tiplab(aes(label = artefact_id), size = 4, offset = 0.01, align = TRUE) +
  theme(plot.margin = margin(40,40,40,40))
