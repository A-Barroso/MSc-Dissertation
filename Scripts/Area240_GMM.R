...
lf <- list.files("Area240Out", full.names = TRUE)

coo <- import_jpg(lf, auto.notcentered = TRUE, 
                  fun.notcentered = NULL, 
                  threshold = 0.5)

database <- read.csv("A240.csv")
...
stack(shapenorm, title = "Stack: Normalised Area 240 Outlines")
...
stack(shapenorm2, title = "Stack: Normalised Area 240 Outlines")
...
tree <- hclust(dist(database.expanded[,12:33]))
