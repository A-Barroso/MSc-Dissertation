...
lf <- list.files("LynfordOut", full.names = TRUE)

coo <- import_jpg(lf, auto.notcentered = TRUE, 
                  fun.notcentered = NULL, 
                  threshold = 0.5)

database <- read.csv("Lynford.csv")
...
stack(shapenorm, title = "Stack: Normalised Lynford Outlines")
...
stack(shapenorm2, title = "Stack: Normalised Lynford Outlines")
...
tree <- hclust(dist(database.expanded[,12:48]))
